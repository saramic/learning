...
- 0 uses 6 segments
- 1 uses 2 segments (unique)
- 2 uses 5 segments
- 3 uses 5 segments
- 4 uses 4 segments (unique)
- 5 uses 5 segments
- 6 uses 6 segments
- 7 uses 3 segments (unique)
- 8 uses 7 segments (unique)
- 9 uses 6 segments
...
def deduce_mapping(patterns)
  {
    1 => patterns.find { _1.length == 2 },
    4 => patterns.find { _1.length == 4 },
    7 => patterns.find { _1.length == 3 },
    8 => patterns.find { _1.length == 7 },
  }.tap do |mapping|
    mapping[3] = patterns.find! { _1.length == 5 && _1.include_all_chars?(mapping[7]) }
    mapping[6] = patterns.select{ _1.length == 6}.reject{ _1.include_all_chars?(mapping[1]) }.only!
    mapping[5] = patterns.find! { _1.length == 5 && mapping[6].include_all_chars?(_1) }
    mapping[2] = patterns.find! { _1.length == 5 && ![mapping[3], mapping[5]].include?(_1) }
    mapping[9] = patterns.find! { _1.length == 6 && _1.include_all_chars?(mapping[5]) && _1 != mapping[6] }
    mapping[0] = patterns.-(mapping.values).only!
  end
end

def sort_patterns(patterns)
  patterns.map { _1.chars.sort.join }
end

class String
  def include_all_chars?(str)
    str.chars.all? { include?(_1) }
  end
end
...
module Enumerable
  ...
  def only!
    raise "Not only: #{to_a.inspect}" unless to_a.size == 1
    first
  end
end

INPUTS = DATA.read.split("\n").map do |line|
  patterns, output_digits = line.split(' | ').map(&:split)
  patterns = sort_patterns(patterns)
  output_digits = sort_patterns(output_digits)
  OpenStruct.new(
    patterns: patterns,
...
puts(
  INPUTS.sum do |inp|
    inp.output_digits.map { inp.mapping.key(_1).to_s }.join.to_i
  end
)
