...
def process_input(line)
  parts = line.split(" | ")
  {
    ...
    output_value: parts[1].split
  }
end

sample1 = <<~EO_SAMPLE1.split("\n").map(&method(:process_input))
  acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab | cdfeb fcadb cdfeb cdbaf
EO_SAMPLE1

sample2 = <<~EO_SAMPLE2.split("\n").map(&method(:process_input))
...
puts "\nPART 1: sample"
pp sample2.map{|line| line[:output_value] }.flatten.count{|digit| SEGEMENTS.values.include?(digit.length) }

puts "\nPART 1: reals"
input= DATA.read.split("\n").map(&method(:process_input))
pp input.map{|line| line[:output_value] }.flatten.count{|digit| SEGEMENTS.values.include?(digit.length) }

def code_for(signal_patterns)
  one_codes = signal_patterns.find{|digit| digit.chars.length == 2 }
  seven_codes = signal_patterns.find{|digit| digit.chars.length == 3 }
  four_codes = signal_patterns.find{|digit| digit.chars.length == 4 }
  code = {}
  code[:a] = (seven_codes.chars - one_codes.chars).first.to_sym
  b_and_d = four_codes.chars - one_codes.chars
  pp b_and_d
  pp seven_codes.chars
  nine_codes = signal_patterns.find_all do |digit|
    digit.chars.length == 6 &&
      (seven_codes.chars - digit.chars).empty? &&
      (b_and_d - digit.chars).empty?
  end.first
  six_codes = signal_patterns.find_all do |digit|
    digit.chars.length == 6 &&
      (seven_codes.chars - digit.chars).length == 1 &&
      (b_and_d - digit.chars).empty?
  end.first
  pp nine_codes
  pp six_codes
  code[:e] = (six_codes.chars - nine_codes.chars).first.to_sym
  code[:c] = (nine_codes.chars - six_codes.chars).first.to_sym
  code
  # {
  #   a: :a,
  #   b: :b,
...
def decode(process_input, codes)
end

puts "\nPART 2: sample1"
code = code_for(sample1.first[:signal_patterns])
pp code
pp decode(sample1.first[:process_input], code)
